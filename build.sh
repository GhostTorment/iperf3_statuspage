...
case "$PROFILE" in
  ...
  release)
    ...
    # Get the binary path
    BIN_PATH="$PROJECT_ROOT/target/release/iperf3_statuspage"
    DEST_PATH="$PROJECT_ROOT/install/usr/local/bin"
    mkdir -p "$DEST_PATH"
    cp "$BIN_PATH" "$DEST_PATH/iperf3-statuspage"
    chmod +x "$DEST_PATH/iperf3-statuspage"
    echo "Binary copied to $DEST_PATH/iperf3-statuspage."
    ...
    IMAGE_NAME="$DOCKER_REPO/iperf3_statuspage"
